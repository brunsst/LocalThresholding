...
namespace hdcom
{
    ...
    /****************** Reading OptFlow-vectors  ******************/
    float * HdCommunication::GetOptFlowResult(std::string rootpath, int outshape[3], bool dspprogress)
    {
    	std::string dx_subdir = "/dx/";
		std::string dy_subdir = "/dy/";
		std::string dz_subdir = "/dz/";

		float* output;

		if (path_exists(rootpath+dx_subdir) && path_exists(rootpath+dy_subdir) && path_exists(rootpath+dz_subdir))
		{
			//Get size
			std::vector<std::string> optflowlist = GetFilelist(rootpath+dx_subdir, outshape);
			std::vector<std::string> dylist = GetFilelist(rootpath+dy_subdir, outshape);
			std::vector<std::string> dzlist = GetFilelist(rootpath+dz_subdir, outshape);

			std::move(dylist.begin(), dylist.end(), std::back_inserter(optflowlist));
			std::move(dzlist.begin(), dzlist.end(), std::back_inserter(optflowlist));

			output = Get3DTifSequence_32bitPointer(optflowlist, outshape, false);
			outshape[2] /= 3;

			if (dspprogress)
				printf("Finished reading 3D-vector of %u x %u x %u voxels \033[1;33m\033[0m\n",outshape[0],outshape[1],outshape[2]);
		}
		else
		{
			struct stat buffer;
			if ((stat ((rootpath+"/ux.tif").c_str(), &buffer) == 0) && (stat ((rootpath+"/uy.tif").c_str(), &buffer) == 0))
			{
				std::vector<std::string> optflowlist = {rootpath+"/ux.tif", rootpath+"/uy.tif"};

				output = Get3DTifSequence_32bitPointer(optflowlist, outshape, false);
				outshape[2] = 1;

				if (dspprogress)
					printf("Finished reading 2D-vector of %u x %u pixels \033[1;33m\033[0m\n",outshape[0],outshape[1]);
			}
			else
			{
				std::cout << "Error! Cannot find valid OptFlow-vector" << std::endl;
				output = (float*) malloc(0);
			}
		}
